...
snr=0:5:10;
sigPow=x*x'/length(x);
y1=zeros(2*length(snr)+1,length(x));
for i=1:length(snr)
noise_mag = sqrt((10.^(-snr(i)/10))*sigPow/2);
rng(5);
y1(i,:)= x + noise_mag*(randn(size(x))+1i*randn(size(x)));
rng(5);
y1(i+length(snr)+1,:)= awgn(x,snr(i),'measured');
end
